...
def main():
    ...
    print('*********************************************************************')
    # Asks the user if he wants to save the selection into a .txt file (to keep track)
    save_choice = input("Would you like to save this selection into a .txt file ? (Enter Y or N) : ")
    print('*********************************************************************')
    # If the user said yes...
    if (save_choice == "Y"):
        # Ask him where he wants to save the .txt file (by entering a destination folder)
        output_path = input("Enter the destination folder of the .txt file : ")
        print('*********************************************************************')
        # Builds the path of the .txt file
        output_file_path = f"{output_path}/random_selection" + " (" + d[0:10] + ").txt"
        # Creates the .txt file in the desired folder
        output_file = open(output_file_path, "w")
        # Writes the selection & the datetime into the .txt file
